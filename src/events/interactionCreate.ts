...
export default async (client: Wrenchi, interaction: Interaction) => {
    ...
    if (interaction.isButton()) {
        const TicketSetup = await TicketSetupSchema.findOne({ ticketGuild: interaction.guild.id });
        ...
        if (interaction.message.id === (await TicketSetup).ticketMessage) {
            ...
            if (!ticket) {
                ...
                interaction.reply({ content: `You already have an opened ticket <#${ticket.channel}>`, ephemeral: true })
                return
            }
            return
        }

        const ticket = await Ticket.findOne({ welcomeMessage: interaction.message.id });
        if (interaction.message.id === (await ticket).welcomeMessage) {
            const ticketsetup = await Ticket.findOne({ channel: interaction.channel.id });
